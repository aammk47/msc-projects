...
inf6028_y2 <- read_excel("Anonymised Student Feedback - Text Comments-20230824T083146Z-001/Anonymised Student Feedback - Text Comments/AY2021-2022/Copy of INF6028 AY2021-2022.xlsx")

View(inf6028_y1)
View(inf6028_y2)

#Rename headings and admire handiwork
inf6028_y1 <- inf6028_y1[, 1:2]
names(inf6028_y1) <- c("Positive", "Negative")

inf6028_y2 <- inf6028_y2[, 1:2]
names(inf6028_y2) <- c("Positive", "Negative")

#Column slicing
##Y1
inf6028_y1.pos <- select(inf6028_y1, Positive)
inf6028_y1.neg <- select(inf6028_y1, Negative)

##Y2
inf6028_y2.pos <- select(inf6028_y2, Positive)
inf6028_y2.neg <- select(inf6028_y2, Negative)

#Tokenization
y1_tidy_dataset_pos = inf6028_y1.pos %>% unnest_tokens(word, Positive)
y1_tidy_dataset_neg = inf6028_y1.neg %>% unnest_tokens(word, Negative)

y2_tidy_dataset_pos = inf6028_y2.pos %>% unnest_tokens(word, Positive)
y2_tidy_dataset_neg = inf6028_y2.neg %>% unnest_tokens(word, Negative)
...
y2_tidy_dataset_pos2$word = y2_tidy_dataset_pos2$word %>%
  str_replace_all("realy", 'really')

y2_tidy_dataset_pos2$word = y2_tidy_dataset_pos2$word %>%
  str_replace_all("lot", '')

y2_tidy_dataset_pos2$word = y2_tidy_dataset_pos2$word %>%
  str_replace_all("alot", '')
...
y2_tidy_dataset_neg2$word = y2_tidy_dataset_neg2$word %>%
  str_replace_all("doesn’t", '')

y2_tidy_dataset_neg2$word = y2_tidy_dataset_neg2$word %>%
  str_replace_all("bb", 'blackboard')
